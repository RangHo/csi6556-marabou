...
class MarabouNetworkTF(MarabouNetwork.MarabouNetwork):
    ...
    def readTF(self, filename, inputNames, outputNames, modelType, savedModelTags):
        ...
        if modelType == "frozen":
            ...
            simp_graph_def = graph_util.convert_variables_to_constants(
                sess, sess.graph.as_graph_def(), outputNames
            )
            with tf.Graph().as_default() as graph:
        ...
        if outputNames:
            ...
            for o in outputNames:
                if o not in valid_names:
                    err_msg = (
                        "The given output name %s is not an operation in the network.\n"
                        % o
                    )
        ...
        if inputNames:
            for i in inputNames:
                if i not in valid_names:
                    err_msg = (
                        "The input name %s is not an operation in the network.\n" % i
                    )
                    err_msg += "Valid names are\n" + ", ".join(valid_names)
                    raise RuntimeError(err_msg)
        ...
        for o in outputOps:
            if o in inputOps:
                raise RuntimeError(
                    "%s cannot be used as both input and output" % (o.node_def.name)
                )
        ...
        if self.foundnInputFlags < len(inputOps):
            ...
            err_msg = (
                "All given inputs are part of the graph, but not all inputs contributed to one of the outputs %s\n"
                % outputNames
            )
        ...
        self.outputVars = [
            self.varMap[outputOp].reshape(self.outputShapes[outputOp])
    ...
    def getScalars(self, op, outputVars):
        ...
        if input1 == op.outputs[0]:
            ...
            self.constantMap[input2.op] = self.sess.run(
                input2, feed_dict=self.dummyFeed
            )
            ...
            sgnScalar = sgn1
            # Extract constants with sess.run since they haven't been added to constantMap yet
            self.constantMap[input1.op] = self.sess.run(
                input1, feed_dict=self.dummyFeed
            )
    ...
    def conv2DEquations(self, op):
        ...
        if padding == "SAME":
            pad_top = (
                max((out_height - 1) * strides_height + filter_height - in_height, 0)
                // 2
            )
            pad_left = (
                max((out_width - 1) * strides_width + filter_width - in_width, 0) // 2
            )
        elif padding == "VALID":
            pad_top = 0
            pad_left = 0
    ...
    def maxpoolEquations(self, op):
        ...
        if padding == "VALID":
            ...
            pad_top = (
                max((out_height - 1) * strides_height + ksize_height - in_height, 0)
                // 2
            )
            pad_left = (
                max((out_width - 1) * strides_width + ksize_width - in_width, 0) // 2
            )
        else:
            raise NotImplementedError(
                "Network uses %s padding for maxpool. Only 'VALID' and 'SAME' padding are currently supported"
                % padding
            )
    ...
    def makeEquations(self, op):
        ...
        # If we've recursed to find a Placeholder operation, this operation needs to be added the inputName list
            ...
            raise RuntimeError(
                "One of the outputs in %s depends on placeholder %s.\nPlease add '%s' to the inputName list."
                % (
                    [
                        self.outputOps[i].node_def.name
                        for i in range(len(self.outputOps))
                    ],
                    op.node_def.name,
                    op.node_def.name,
                )
    ...
    def buildEquations(self, op):
        ...
        if not self.isVariable(op):
            self.constantMap[op] = self.sess.run(
                op.outputs[0], feed_dict=self.dummyFeed
            )
            return
    ...
    def evaluateWithoutMarabou(self, inputValues):
        ...
        if len(inputValues) != len(self.inputOps):
            raise RuntimeError(
                "Bad input given. Please provide a list of %d np arrays"
                % len(self.inputOps)
            )
        ...
        for i, (op, inputVals) in enumerate(zip(self.inputOps, inputValues)):
            ...
            if np.prod(opShape) != np.prod(inputVals.shape):
                raise RuntimeError(
                    "Input %d should have shape " % i
                    + str(opShape)
                    + ", or be reshapeable to this shape, but an array with shape "
                    + str(inputVals.shape)
                    + " was given"
                )
        ...
        out = [
            self.sess.run(outputOp.outputs[0], feed_dict=feed_dict)
            for outputOp in self.outputOps
        ]

        # Make sure to return output in correct shape
        return [
            out[i].reshape(self.varMap[outputOp].shape)
            for i, outputOp in enumerate(self.outputOps)
        ]
