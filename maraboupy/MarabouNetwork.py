...
class MarabouNetwork(InputQueryBuilder):
            ...
            if exitCode == "sat":
                for j in range(len(self.inputVars)):
                    for i in range(self.inputVars[j].size):
                        print(
                            "input {} = {}".format(i, vals[self.inputVars[j].item(i)])
                        )

                for j in range(len(self.outputVars)):
                    for i in range(self.outputVars[j].size):
                        print(
                            "output {} = {}".format(i, vals[self.outputVars[j].item(i)])
                        )
    ...
    def calculateBounds(self, filename="", verbose=True, options=None):
        ...
        if options == None:
            options = MarabouCore.Options()
        exitCode, bounds, stats = MarabouCore.calculateBounds(
            ipq, options, str(filename)
        )
        ...
        if verbose:
            ...
            if exitCode == "":
                for j in range(len(self.outputVars)):
                    for i in range(self.outputVars[j].size):
                        print(
                            "output bounds {} = {}".format(
                                i, bounds[self.outputVars[j].item(i)]
                            )
                        )

        return [exitCode, bounds, stats]

    def evaluateLocalRobustness(
        self,
        input,
        epsilon,
        originalClass,
        ...
        if type(self.inputVars) is list:
            if len(self.inputVars) != 1:
                raise NotImplementedError(
                    "Operation for %d inputs is not implemented" % len(self.inputVars)
                )
        ...
        if type(self.outputVars) is list:
            if len(self.outputVars) != 1:
                raise NotImplementedError(
                    "Operation for %d outputs is not implemented" % len(self.outputVars)
                )
        ...
        if targetClass is None:
            ...
            self.addMaxConstraint(
                set(self.outputVars[0][0]), outputStartIndex + targetClass
            )
            exitCode, vals, stats = self.solve(options=options)
            if verbose:
    ...
    def evaluateWithMarabou(
        self, inputValues, filename="evaluateWithMarabou.log", options=None
    ):
        ...
        inputDict = dict()
        inputVarList = np.concatenate(
            [inVar.flatten() for inVar in inputVars], axis=-1
        ).flatten()
        inputValList = np.concatenate(
            [inVal.flatten() for inVal in inputValues]
        ).flatten()
    ...
    def evaluate(
        ...
        """
        if useMarabou:
            return self.evaluateWithMarabou(
                inputValues, filename=filename, options=options
            )
        if not useMarabou:
    ...
    def findError(self, inputValues, options=None, filename="evaluateWithMarabou.log"):
        ...
        """
        outMar = self.evaluate(
            inputValues, useMarabou=True, options=options, filename=filename
        )
        outNotMar = self.evaluate(
            inputValues, useMarabou=False, options=options, filename=filename
        )
