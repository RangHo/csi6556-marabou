...
def test_split_onnx():
    ...
    split_successful = network.splitNetworkAtNode(
        split_point, presplit_filename, postsplit_filename
    )
...
def test_split_onnx_error():
    filename = os.path.join(
        os.path.dirname(__file__), NETWORK_FOLDER, "oneInput_twoBranches.onnx"
    )
    network = Marabou.read_onnx(filename)
    presplit_filename = "test_presplit.onnx"
    postsplit_filename = "test_postsplit.onnx"
...
def test_intermediate_input_output():
    ...
    evaluateIntermediateLayers(
        filename, inputNames=["X"], outputNames=["Y"], intermediateNames=["11"]
    )
...
def test_twoBranches():
    ...
    evaluateIntermediateLayers(
        filename, inputNames=["X"], outputNames=["Y"], intermediateNames=["add1"]
    )
...
def test_multiInput_add(tmpdir):
    ...
    network_oneIn = Marabou.read_onnx(
        filename, inputNames=["X1"], outputNames=["relu1"]
    )
    network_twoIn = Marabou.read_onnx(
        filename, inputNames=["X1", "X2"], outputNames=["relu1"]
    )

    testInputs = [
        [np.random.random(inVars.shape) for inVars in network_twoIn.inputVars]
    ...
    for testInput in testInputs:
        marabouEval_oneIn = network_oneIn.evaluateWithMarabou(
            [testInput[0]], filename=tempFile
        )[0].flatten()
        marabouEval_twoIn = network_twoIn.evaluateWithMarabou(
            testInput, options=OPT, filename=""
        )[0].flatten()
...
def evaluateFile(
    filename, inputNames=None, outputNames=None, testInputs=None, numPoints=NUM_RAND
):
    """
    ...
    network = Marabou.read_onnx(
        filename, inputNames=inputNames, outputNames=outputNames
    )
    ...
    if not testInputs:
        testInputs = [
            [np.random.random(inVars.shape) for inVars in network.inputVars]
...
def evaluateIntermediateLayers(
    filename,
    inputNames=None,
    outputNames=None,
    intermediateNames=None,
    testInputs=None,
    numPoints=None,
):
    """
    ...
    """
    filename = os.path.join(os.path.dirname(__file__), NETWORK_FOLDER, filename)
    fullNetwork = Marabou.read_onnx(
        filename, inputNames=inputNames, outputNames=outputNames
    )
    startNetwork = Marabou.read_onnx(
        filename, inputNames=inputNames, outputNames=intermediateNames
    )
    endNetwork = Marabou.read_onnx(
        filename, inputNames=intermediateNames, outputNames=outputNames
    )

    testInputs = [
        [np.random.random(inVars.shape) for inVars in fullNetwork.inputVars]
        for _ in range(NUM_RAND)
    ]
    for testInput in testInputs:
        marabouEval_full = fullNetwork.evaluateWithMarabou(
            testInput, options=OPT, filename=""
        )
        ...
        for i in range(len(marabouEval_full)):
            assert (
                max(abs(marabouEval_full[i].flatten() - onnxEval_full[i].flatten()))
                < TOL
            )

        marabouEval_start = startNetwork.evaluateWithMarabou(
            testInput, options=OPT, filename=""
        )
        ...
        for i in range(len(marabouEval_start)):
            assert (
                max(abs(marabouEval_start[i].flatten() - onnxEval_start[i].flatten()))
                < TOL
            )

        marabouEval_end = endNetwork.evaluateWithMarabou(
            [marabouEval_start], options=OPT, filename=""
        )
        assert len(marabouEval_end) == len(onnxEval_full)
        for i in range(len(marabouEval_end)):
            assert (
                max(abs(marabouEval_end[i].flatten() - onnxEval_full[i].flatten()))
                < TOL
            )
