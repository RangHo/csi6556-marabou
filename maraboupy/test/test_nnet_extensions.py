...
NETWORK_FILENAME = os.path.join(
    os.path.dirname(__file__), NETWORK_FOLDER, "acasxu/ACASXU_experimental_v2a_1_9.nnet"
)
...
def test_evaluate_network():
    ...
    for i in range(N):
        ...
        output1 = nnet_object.evaluateNNet(
            inputs,
            last_layer=-1,
        ...
        output2 = nnet_object.evaluateNNet(
            inputs, normalize_inputs=False, normalize_outputs=False
        )

        assert (output1 == output2).all()

        # Compare evaluation with and without Marabou
        without_marabou_output = nnet_object.evaluate(
            np.array([inputs]), useMarabou=False
        )[0].flatten()
        without_marabou_output_rounded = np.array(
            [float(round(y, 8)) for y in without_marabou_output]
        )

        with_marabou_output = nnet_object.evaluate(np.array([inputs]), useMarabou=True)[
            0
        ].flatten()
        with_marabou_output_rounded = np.array(
            [float(round(y, 8)) for y in with_marabou_output]
        )
        ...
        output1 = nnet_object.evaluateNNet(
            layer_output, first_layer=LAYER, normalize_outputs=True
        )
...
def test_write_read_evaluate(tmpdir):
    ...
    """
    output_filename = (
        tmpdir.mkdir("output_network")
        .join("ACASXU_experimental_v2a_1_9_output.nnet")
        .strpath
    )
    ...
    for i in range(N):
        ...
        output2 = nnet_object_a.evaluateNNet(
            inputs, normalize_inputs=False, normalize_outputs=False
        )
        assert (output1 == output2).all()
        ...
        with_marabou_output = nnet_object_a.evaluate(
            np.array([inputs]), useMarabou=True
        )[0].flatten()
...
def test_reset_network():
    ...
    nnet_object2.resetNetworkFromParameters(
        weights=nnet_object1.weights, biases=nnet_object1.biases
    )
    ...
    for i in range(N):
        ...
        output1 = nnet_object1.evaluateNNet(
            inputs, normalize_inputs=False, normalize_outputs=False
        )
        output2 = nnet_object2.evaluateNNet(
            inputs, normalize_inputs=False, normalize_outputs=False
        )

        # Normalization should not matter with default parameters
        output2_norm = nnet_object2.evaluateNNet(
            inputs, normalize_inputs=True, normalize_outputs=True
        )
    ...
    nnet_object2.resetNetworkFromParameters(
        weights=nnet_object1.weights,
        biases=nnet_object1.biases,
    ...
    for i in range(N):
        ...
        output1_norm = nnet_object1.evaluateNNet(
            inputs, normalize_inputs=True, normalize_outputs=True
        )
        output2_norm = nnet_object2.evaluateNNet(
            inputs, normalize_inputs=True, normalize_outputs=True
        ...
        without_marabou_output1 = nnet_object1.evaluate(
            np.array([inputs]), useMarabou=False
        )[0].flatten()
        with_marabou_output1 = nnet_object1.evaluate(
            np.array([inputs]), useMarabou=True
        )[0].flatten()
        without_marabou_output2 = nnet_object2.evaluate(
            np.array([inputs]), useMarabou=False
        )[0].flatten()
        with_marabou_output2 = nnet_object2.evaluate(
            np.array([inputs]), useMarabou=True
        )[0].flatten()
...
def test_bound_getters():
    ...
    assert (
        nnet_object.getVariable(1, 1, b=False)
        == num_input_vars + nnet_object.layerSizes[1] + 1
    )
    ...
    if not nnet_object.upperBoundExists(var_b):
        ...
        assert (
            nnet_object.getUpperBound(random_hidden_layer, random_node, b=False) is None
        )
        assert (
            nnet_object.getUpperBound(random_hidden_layer, random_node, b=True) is None
        )
