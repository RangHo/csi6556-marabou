            ...
            for k in range(len(inputVars[i][j])):
                for l in range(len(inputVars[i][j][k])):
                    network.setLowerBound(
                        inputVars[i][j][k][l], inputValues[i][j][k][l]
                    )
        ...
        for j in range(len(outputVars[i])):
            for k in range(len(outputVars[i][j])):
                for l in range(len(outputVars[i][j][k])):
                    assert (
                        abs(
                            vals[outputVars[i][j][k][l]]
                            - expectedOutputValues[i][j][k][l]
                        )
                        < TOL
                    )
...
def test_local_robustness_unsat():
    ...
    vals, stats, maxClass = network.evaluateLocalRobustness(
        input=input, epsilon=0.1, originalClass=0, options=options, targetClass=None
    )
...
def test_local_robustness_sat_of_onnx():
    ...
    input = np.array([1, 0])
    vals, stats, maxClass = network.evaluateLocalRobustness(
        input=input, epsilon=1.0, originalClass=0, options=options, targetClass=None
    )
...
def evaluateNetwork(network, testInputs, testOutputs):
    ...
    for testInput, testOutput in zip(testInputs, testOutputs):
        marabouEval = network.evaluateWithMarabou(
