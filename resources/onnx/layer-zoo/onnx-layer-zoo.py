...
def flatten_node():
    node = onnx.helper.make_node(
        "Flatten", inputs=[input_name], outputs=[output_name], axis=2
    )
...
def transpose_node():
    node = onnx.helper.make_node(
        "Transpose", inputs=[input_name], outputs=[output_name], perm=[1, 0]
    )
    return ("transpose", node, [2, 3], [3, 2], [])


def squeeze_node():
    node = onnx.helper.make_node("Squeeze", inputs=[input_name], outputs=[output_name])
...
def squeeze_node_with_axes():
    ...
    node = onnx.helper.make_node(
        "Squeeze", inputs=[input_name, "axes"], outputs=[output_name]
    )
...
def unsqueeze_node():
    ...
    node = onnx.helper.make_node(
        "Unsqueeze", inputs=[input_name, "axes"], outputs=[output_name]
    )
...
def conv_node():
    weights = make_constant_float_node(
        "weights_const",
        [
            [
                [
...
def leakyRelu_node():
    node = onnx.helper.make_node(
        "LeakyRelu", inputs=[input_name], outputs=[output_name], alpha=0.05
    )
...
def matmul_node():
    const_node = make_constant_float_node(
        "const", np.array([[0.0, 0.5], [1.5, 2.0], [-1, -2]])
    )
...
def dropout_node():
    node = onnx.helper.make_node("Dropout", inputs=[input_name], outputs=[output_name])
...
def dropout_training_mode_false_node():
    ...
    node = onnx.helper.make_node(
        "Dropout", inputs=[input_name, "ratio", "trainingMode"], outputs=[output_name]
    )
