...
def main():
    ...
    if query == None:
        ...
        print(
            "There are three options to define the benchmark:\n"
...
def encode_cifar10_linf(network, index, epsilon, target_label):
    ...
    cifar_test = datasets.CIFAR10(
        "./data/cifardata/", train=False, download=True, transform=transforms.ToTensor()
    )
...
def arguments():
    ...
    marabou_path = os.path.join(
        str(pathlib.Path(__file__).parent.absolute()), "../build/Marabou"
    )
    parser.add_argument(
        "--marabou-binary",
        type=str,
